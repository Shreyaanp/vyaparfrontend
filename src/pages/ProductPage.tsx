...
const ProductPage = () => {
  ...
  const addProductData = async () => {
    if (productData) {
      ...
      try {
        const response = await axios.post(`${backendUrl}product/upload`, productData);
  ...
  const updateProductData = async () => {
    if (productData && productData._id) {
      try {
        const response = await axios.put(`${backendUrl}product/${productData._id}`, productData);
  ...
  const publishProduct = async () => {
    try {
      const response = await axios.post(`${backendUrl}product/publish`, { uid: productData.uid, data: productData });
      ...
      <div className="flex flex-1 flex-row px-10 justify-center gap-24 mb-5">
        <div className="flex flex-col w-[37rem]">
          <div className="w-[33rem]">
            <div onClick={() => navigate('/ecommerce')} className="m-5 cursor-pointer">
